...
namespace ImageClassification
{
    ...
    public class ImageClass
    {
        ...
        public ImageClass(string modelPath = (@"C:\Users\Администратор\source\repos\ImageClassification\ImageClassification\resnet34-v2-7.onnx"))
        {
            try
            {
        ...
        public void Process(string imagePath, int tmp)
        {
            ...
            using (var image = Image.Load<Rgb24>((string)imagePath ?? "image.jpg"))
            {
                ...
                using (IDisposableReadOnlyCollection<DisposableNamedOnnxValue> results = Session.Run(inputs))
                {
                    ...
                    Notify?.Invoke(new StringPath((string)imagePath, LabelClass.Labels[index]), new EventArgs());
                }
            }
        }


        public delegate void AccountHandler(StringPath sender, EventArgs e);
